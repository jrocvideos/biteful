...
import { MapPin, CreditCard, Clock, ChevronRight, Minus, Plus, Trash2, Tag, Bike, Shield, Star } from 'lucide-react';
          ...
          <div className="lg:col-span-3 space-y-6">
                    ...
                    <div className="space-y-4">
                      {groupItems.map((item) => (
                        <div key={`${item.id}-${item.restaurantId}`} className="flex items-center gap-4">
                          <div className="w-20 h-20 rounded-xl bg-muted overflow-hidden flex-shrink-0"><img src={item.image} alt={item.name} className="w-full h-full object-cover" /></div>
                          <div className="flex-1 min-w-0"><h4 className="font-semibold truncate">{item.name}</h4><p className="text-sm text-muted-foreground">${item.price.toFixed(2)} each</p></div>
                          ...
                          <div className="text-right min-w-[80px]"><p className="font-bold">${(item.price * item.quantity).toFixed(2)}</p><button onClick={() => onRemove(item.id, item.restaurantId)} className="text-red-500 hover:text-red-600"><Trash2 className="w-4 h-4" /></button></div>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
                ...
                <div className="bg-card rounded-2xl border border-border p-6 space-y-6">
                  <div>
                    ...
                    <div className="relative"><MapPin className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground" /><input type="text" value={address} onChange={(e) => setAddress(e.target.value)} placeholder="Enter your street address" className="w-full pl-10 pr-4 py-3 rounded-xl bg-muted border border-border focus:outline-none focus:ring-2 focus:ring-primary/50" /></div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <button onClick={() => setDeliveryTime('asap')} className={`p-4 rounded-xl border-2 text-center transition-colors ${deliveryTime === 'asap' ? 'border-primary bg-primary/5' : 'border-border hover:bg-muted'}`}><Clock className="w-6 h-6 mx-auto mb-2 text-primary" /><p className="font-semibold">ASAP</p><p className="text-xs text-muted-foreground">~25-35 min</p></button>
                      <button onClick={() => setDeliveryTime('schedule')} className={`p-4 rounded-xl border-2 text-center transition-colors ${deliveryTime === 'schedule' ? 'border-primary bg-primary/5' : 'border-border hover:bg-muted'}`}><Clock className="w-6 h-6 mx-auto mb-2 text-primary" /><p className="font-semibold">Schedule</p><p className="text-xs text-muted-foreground">Pick a time</p></button>
                    </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-5 gap-2">{[0, 0.10, 0.15, 0.20, 0.25].map((t) => <button key={t} onClick={() => setTip(t)} className={`py-2 rounded-xl text-sm font-medium border transition-colors ${tip === t ? 'bg-primary text-primary-foreground border-primary' : 'bg-muted border-border hover:bg-muted/80'}`}>{t === 0 ? 'None' : `${Math.round(t * 100)}%`}</button>)}</div>
            ...
            {step === 'payment' && (
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
                ...
                <div className="bg-card rounded-2xl border border-border p-6 space-y-6">
                  <div className="grid grid-cols-3 gap-3">{[{ id: 'card', icon: CreditCard, label: 'Card' }, { id: 'apple', icon: CreditCard, label: 'Apple Pay' }, { id: 'google', icon: CreditCard, label: 'Google Pay' }].map((m) => <button key={m.id} onClick={() => setPaymentMethod(m.id as any)} className={`p-4 rounded-xl border-2 text-center transition-colors ${paymentMethod === m.id ? 'border-primary bg-primary/5' : 'border-border hover:bg-muted'}`}><m.icon className="w-6 h-6 mx-auto mb-2 text-primary" /><p className="font-semibold text-sm">{m.label}</p></button>)}</div>
                  {paymentMethod === 'card' && <div className="space-y-4"><div><label className="block text-sm font-semibold mb-2">Card Number</label><input type="text" placeholder="1234 5678 9012 3456" className="w-full px-4 py-3 rounded-xl bg-muted border border-border focus:outline-none focus:ring-2 focus:ring-primary/50" /></div><div className="grid grid-cols-2 gap-4"><div><label className="block text-sm font-semibold mb-2">Expiry</label><input type="text" placeholder="MM/YY" className="w-full px-4 py-3 rounded-xl bg-muted border border-border focus:outline-none focus:ring-2 focus:ring-primary/50" /></div><div><label className="block text-sm font-semibold mb-2">CVC</label><input type="text" placeholder="123" className="w-full px-4 py-3 rounded-xl bg-muted border border-border focus:outline-none focus:ring-2 focus:ring-primary/50" /></div></div></div>}
                  <div className="flex items-center gap-2 p-3 bg-green-500/10 rounded-xl"><Shield className="w-5 h-5 text-green-500" /><p className="text-sm text-green-700">Your payment is secured with 256-bit encryption</p></div>
                </div>
                <div className="flex gap-3 mt-6">
                  ...
                  <button onClick={handlePlaceOrder} disabled={loading} className="flex-1 py-4 bg-primary text-primary-foreground rounded-xl font-bold text-lg hover:bg-primary/90 transition-colors disabled:opacity-70">{loading ? 'Placing Order...' : `Place Order • $${finalTotal.toFixed(2)}`}</button>
                </div>
              </motion.div>
            )}
          </div>
            ...
            <div className="sticky top-28 bg-card rounded-2xl border border-border p-6">
              ...
              <div className="mb-4">
                <div className="flex gap-2">
                  <div className="relative flex-1"><Tag className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" /><input type="text" value={promoCode} onChange={(e) => setPromoCode(e.target.value)} placeholder="Promo code" className="w-full pl-9 pr-3 py-2 rounded-lg bg-muted border border-border text-sm focus:outline-none focus:ring-2 focus:ring-primary/50" /></div>
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="flex justify-between"><span className="text-muted-foreground">Service Fee (10%)</span><span>${serviceFee.toFixed(2)}</span></div>
                <div className="flex justify-between"><span className="text-muted-foreground">Tax (12%)</span><span>${tax.toFixed(2)}</span></div>
                <div className="flex justify-between"><span className="text-muted-foreground">Driver Tip</span><span>${tipAmount.toFixed(2)}</span></div>
